...
e.offset=this.offset;e.scale.off=this.scale.off;e.scale.pretty=this.scale.pretty;e.laneSizes=this.laneSizes;e.drawStyle=this.drawStyle;e.glyph=this.glyph;e.glyphHooks=this.glyphHooks;e.trackHooks=this.trackHooks;e.previousDrawStyle=this.previousDrawStyle;for(var f=0;f<d;f++){var g=this.tracks[f],h=e.addTrack();h.drawStyle=g.drawStyle;for(var i=g.lanes.length,l=0;l<i;l++)for(var k=h.addLane(),j=g.lanes[l].features,m=0;m<j.length;m++){var n=j[m].position+j[m].length,o=j[m].position;if(c=="inclusive")o>=
a&&o<=b?k.addFeature(j[m].clone()):n>a&&n<b?k.addFeature(j[m].clone()):o<a&&n>b?k.addFeature(j[m].clone()):o>a&&n<b&&k.addFeature(j[m].clone());else if(c=="strict")if(o>=a&&o<=b)if(n>a&&n<b)k.addFeature(j[m].clone());else{var p=j[m].glyphType=="BlockArrow"&&j[m].strand=="+"?j[m].clone("Rect"):j[m].clone();p.length=Math.abs(b-o);k.addFeature(p)}else if(n>a&&n<b)p=j[m].glyphType=="BlockArrow"&&j[m].strand=="-"?j[m].clone("Rect"):j[m].clone(),p.position=a,p.length=Math.abs(n-a),k.addFeature(p);else{if(o<
a&&n>b)p=j[m].glyphType=="BlockArrow"?j[m].clone("Rect"):j[m].clone(),p.position=a,p.length=Math.abs(b-a),k.addFeature(p)}else c=="exclusive"&&o>=a&&o<=b&&n>a&&n<b&&k.addFeature(j[m].clone())}}return e},draw:function(){var a=this.ctx,b=this.tracks;this.scrollable==!0&&this.initScrollable();a.save();this.initScale();if(this.offset==void 0)this.offset=Math.ceil(a.measureText("0").width/2+10);a.save();for(var c=0;c<b.length;c++)!this.scale.off&&this.scale.positions.indexOf(c)!=-1&&this.drawScale(),b[c].draw();
...
this.ctx.font=e+"px "+d;d=this.ctx.measureText(f);var g=[f],h=e+10,i=d.width+10,l,k;d=0;a.seq&&(d=this.ntOffset*(a.getPixelLength()/a.length));d=a.getPixelPositionX()+this.horizontalOffset+d;var j;j=this.placement=="below"?a.getPixelPositionY()+a.getHeight()-this.verticalOffset:a.getPixelPositionY()-h-this.verticalOffset;a.getPixelLength();if(i>200)g=this.ctx.measureText("s").width,g=parseInt(200/g),f=ScriblWrapLines(g,f),i=210,h=f[1]*e+10,g=f[0];i+d>this.chart.width&&(d=this.chart.width-i);if(this.chart.tooltips.style==
"light")l=this.chart.ctx.createLinearGradient(d+i/2,j,d+i/2,j+h),l.addColorStop(0,"rgb(253, 248, 196)"),l.addColorStop(0.75,"rgb(253, 248, 196)"),l.addColorStop(1,"white"),k=this.chart.ctx.createLinearGradient(d+i/2,j,d+i/2,j+h),k.addColorStop(0,"black"),k.addColorStop(1,"rgb(64, 64, 64)"),this.chart.tooltips.text.color="black";else if(this.chart.tooltips.style=="dark")l=this.chart.ctx.createLinearGradient(d+i/2,j,d+i/2,j+h),l.addColorStop(0,"rgb(64, 64, 64)"),l.addColorStop(1,"rgb(121, 121, 121)"),
k="white",this.chart.tooltips.text.color="white";this.chart.lastToolTips.push({pixels:this.ctx.getImageData(d-1,j-1,i+2,h+2),x:d-1,y:j-1});this.ctx.fillStyle=l;this.ctx.beginPath();tlc_ctrl_x=d;tlc_ctrl_y=j;tlc_lgth_x=d+c;tlc_lgth_y=j;tlc_wdth_x=d;tlc_wdth_y=j+c;blc_ctrl_x=d;blc_ctrl_y=j+h;blc_lgth_x=d+c;blc_lgth_y=j+h;blc_wdth_x=d;blc_wdth_y=j+h-c;brc_ctrl_x=d+i;brc_ctrl_y=j+h;brc_lgth_x=d+i-c;brc_lgth_y=j+h;brc_wdth_x=d+i;brc_wdth_y=j+h-c;trc_ctrl_x=d+i;trc_ctrl_y=j;trc_lgth_x=d+i-c;trc_lgth_y=
...
g=SCRIBL.chars;if(!g.heights[c]){g.heights[c]=[];for(var h=0;h<this.chars.list.length;h++){var i=this.chars.list[h],l=i;i=="-"&&(l="dash");this.createChar(i,g.nt_color,g["nt_"+l+"_bg"],c)}}x=y=0;if(this.imgCanvas)a.drawImage(this.imgCanvas,e,f-c*d,b,c*d);else{a.save();a.beginPath();a.textBaseline="middle";e=a.font;h=/[\d+px]/.exec(e)+"px";a.font=h+" courier";a.fillStyle="black";a.textAlign="left";h=this.seq.length*g.heights[c].width;this.imgCanvas=document.createElement("canvas");this.imgCanvas.width=
h;this.imgCanvas.height=c;f=this.imgCanvas.getContext("2d");for(h=l=i=0;h<this.seq.length;h++){g.heights[c][this.seq[h]]||this.createChar(this.seq[h],"black","white",c);var k=this.seq[h];this.insertions[l]&&this.insertions[l].pos!=void 0&&(this.insertions[l].pos-1==h?k+="rightInsert":this.insertions[l]&&this.insertions[l].pos==h&&(k+="leftInsert",l++));f.drawImage(g.heights[c][k],i,y);i+=g.heights[c].width}a.drawImage(this.imgCanvas,x,c-c*d,b,c*d);a.font=e;a.restore()}a.beginPath();a.moveTo(0,0);
...
a_ctrl_y,a_t_x,a_t_y);ts_ctrl_y=y;ts_ctrl_x=(ts_ctrl_y+ts_intercept)/this.slope;ts_slpe_x=ts_ctrl_x+d+d;ts_slpe_y=-(ts_slope*ts_slpe_x+ts_intercept);a.lineTo(ts_slpe_x,ts_slpe_y);var e=Math.PI-Math.abs(Math.atan(this.slope))-Math.PI/2;d=Math.sin(e)*b;e=Math.cos(e)*b;a.bezierCurveTo(ts_ctrl_x,ts_ctrl_y,ts_ctrl_x-d,ts_ctrl_y+e,ts_slpe_x-d,ts_slpe_y+e);a.lineTo(a_max_x-b,y+c/2);a.lineTo(bs_slpe_x-d,bs_slpe_y-e);a.bezierCurveTo(bs_ctrl_x-d,bs_ctrl_y-e,bs_ctrl_x,bs_ctrl_y,bs_slpe_x,bs_slpe_y)}});function genbank(a,b){try{for(var c=a.split("\n"),d=RegExp(/\s+gene\s+([a-z]*)\(?(\d+)\.\.(\d+)/),e=[],f=void 0,g=void 0,h=0;h<c.length;h++){var i;if(i=c[h].match(d)){i.shift();e.push(i);var l=i[2];if(f==void 0||f>l)f=l;var k=i[1];if(g==void 0||g<k)g=k}}b.scale.max=f;b.scale.min=g;for(c=0;c<e.length;c++)d="+",e[c][0]=="complement"&&(d="-"),k=e[c][1],l=e[c][2],k=k-1+1,b.addGene(k,l-k,d)}catch(j){throw"Parsing Error: could not parse genbank file";}};function bed(a,b){try{var c=a.split("\n");numFeatures=c.length;for(var d=1;d<numFeatures;d++){if(c[d]=="")break;for(var e=c[d].split(/\s+/),f=parseInt(e[1]),g=parseInt(e[2]),h=e[0]+": "+e[3],i=e[5],l=e[8],k=e[10].split(","),j=e[11].split(","),m=b.addFeature(new Complex("complex",f,g,i,[],{color:l,name:h})),n=0;n<k.length;n++){if(k[n]=="")break;m.addSubFeature(new BlockArrow("complex",parseInt(j[n]),parseInt(k[n]),i))}}}catch(o){throw"Parsing Error: could not parse bed file";}};
